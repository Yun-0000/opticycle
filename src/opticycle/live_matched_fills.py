"""Two real Monday paper broker fills (live_paper). Do not invent extras.

Record broker facts only. Omit Alpaca account id. No keys. Replay MATCHED
stays a fixture. These two episodes filled at the broker but were submitted
with a debit-signed (positive) MLEG limit, so they are not price-bound
MATCHED under Alpaca's debit+/credit- convention.
"""
