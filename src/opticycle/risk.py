...
The hardcoded synthetic fallback limit is never a risk input. The LLM cannot
...
# Regression sentinel. Never used as a quote, premium, or certificate price.
FORBIDDEN_SYNTHETIC_LIMIT = Decimal("0.85")
...
class RiskEngine:
    ...
    def _evaluate(
        ...
        # Never let a synthetic fallback masquerade as a market price inside the cert.
        _assert_no_synthetic_fallback_price(calculated, quotes)
...
def _assert_no_synthetic_fallback_price(
    calculated: CalculatedRisk,
    quotes: Mapping[str, OptionContractQuote],
) -> None:
    """Reject invalid quotes; risk inputs must remain market-derived."""
    _ = calculated
