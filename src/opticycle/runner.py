...
from opticycle.verticals import (
    MarketContext,
...
from opticycle.replay_market import replay_market
...
def _market_from_observation(obs: ObservationResult) -> MarketContext:
...
def run_once(
    ...
    *,
    ...
    if not dry_run:
        ...
        market_context = _market_from_observation(observation)
        ...
        spot = float(market_context.spot)
        try:
            plan = build_cycle_plan(
                settings,
                market=market_context,
    ...
    if dry_run:
        evidence = evidence_from_chain_rows(
            ...
            rows=market_context.chain,
            account_id=account_id,
            timestamp=datetime.now(timezone.utc),
            quote_age_seconds=Decimal("0"),
            bars_count=int(len(market_context.bars)),
        )
