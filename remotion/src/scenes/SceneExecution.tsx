...
export const SceneExecution: React.FC<{durationInFrames?: number}> = ({durationInFrames = 240}) => {
  ...
  return (
    <SceneFrame durationInFrames={durationInFrames} index="ZERO RESUBMIT" accent={c.red}>
      ...
      <div style={{position: "absolute", right: 76, top: 248, width: 490, opacity: show(frame, 128, 16)}}>
        ...
        <div style={{fontSize: 26, marginTop: 16, letterSpacing: -1.5}}>{golden.clientOrderId}</div>
      ...
      <div style={{position: "absolute", left: 76, bottom: 82, display: "flex", gap: 56, opacity: show(frame, 154, 14)}}>
        <Label>MCP SUBMIT COUNT {golden.mcpSubmitCount}</Label><Label color={c.red}>SECOND SUBMIT {golden.secondSubmit}</Label>
      </div>
    </SceneFrame>
  );
};
