...
export const SceneRisk: React.FC<{durationInFrames?: number}> = ({durationInFrames = 240}) => {
  ...
  const hash = golden.payloadHash;
  const lock = show(frame, 84, 18);
  return (
    <SceneFrame durationInFrames={durationInFrames} index="PAYLOAD CERTIFICATE" accent={c.yellow}>
      ...
      <div style={{position: "absolute", left: 72, top: 230, width: 470, opacity: show(frame, 20, 16)}}>
        <Label color={c.red}>SELL {golden.sellLeg}</Label><div style={{height: 1, background: c.red, marginTop: 18}} />
        <Label color={c.white} style={{marginTop: 58}}>BUY {golden.buyLeg}</Label><div style={{height: 1, background: c.white, marginTop: 18}} />
        <Label color={c.gray} style={{marginTop: 54}}>QTY {golden.quantity} · LIMIT {golden.limit}</Label>
      </div>
      <div style={{position: "absolute", left: 640, top: 200, width: 540, height: 420, border: `1px solid ${c.white}`, opacity: show(frame, 42, 18)}}>
        ...
        <div style={{position: "absolute", left: 24, right: 24, bottom: 24, display: "flex", justifyContent: "space-between"}}><Label>MAX LOSS {golden.maxLoss}</Label><Label>MCP MLEG</Label><Label>SPY</Label></div>
      </div>
