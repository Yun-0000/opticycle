...
export const SceneRunOnce: React.FC<{durationInFrames?: number}> = ({durationInFrames = 210}) => {
  ...
  return (
    <SceneFrame durationInFrames={durationInFrames} index="GOLDEN TRACE" accent={c.acid}>
      ...
      <Label color={c.gray} style={{position: "absolute", left: 72, bottom: 112, opacity: show(frame, 118, 14)}}>SNAPSHOT → LLM STANCE → PAYLOAD → CERT → MCP → GET → MATCHED</Label>
      <Label color={c.acid} style={{position: "absolute", left: 72, bottom: 68, opacity: show(frame, 138, 14)}}>ARTIFACTS / EVIDENCE / INDEX.HTML</Label>
    </SceneFrame>
  );
};
